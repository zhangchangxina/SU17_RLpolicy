...
CBF_SAFE_DIST=3.0       # Barrier 函数作用距离 (米), 进入此范围 barrier 值开始升高
...
CBF_REPULSION_DIST=3.0    # 硬排斥触发距离 (米), 小于此距离施加紧急排斥力
CBF_REPULSION_GAIN=2.0   # 硬排斥力增益 (0=关闭, 0.5=轻柔, 2.0=原始强度)
                      #   手动设值会覆盖自动选择，越小越保守
...
# 目标点附近减速/悬停模式
# none  = 不减速, 策略全程控制
# decel = 指数减速 (XY 1m内指数衰减, 先慢后快, 障碍物<2m取消减速)
# hover = 悬停 (XY<1m悬停, 滞回1.5m退出, 障碍物<2m取消悬停)
# ============================================================
TARGET_STOP_MODE=decel
